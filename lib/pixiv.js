...
(function() {
  ...
  pixiv = {
    ...
    searchWorks: function(session, mode, order, pageNo, keyword, callback) {
      var params, url;
      ...
      return request.get(url, function(error, resp, body) {
        if (error) {
          callback(error);
          return;
        }
        body = body.slice(0, -1);
        return callback(null, (body.split("\n")).map(function(r) {
          return Work.parseSingle(r);
        }));
      });
    }
  };
